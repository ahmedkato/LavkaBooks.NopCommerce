...
namespace Nop.Web.Controllers
{
	public partial class CatalogController : BaseNopController
	{
		...
		{
			if (level > 5)
				return new List<CategoryNavigationModel.CategoryModel>();
				...
				var model = _specificationAttributeService.GetProductSpecificationAttributesByProductId(product.Id, null, true)
				   .Select(psa =>
				   {
					   return new ProductSpecificationModel()
					   {
						   ...
						   SpecificationAttributeOrder = psa.SpecificationAttributeOption.SpecificationAttribute.DisplayOrder
					   };
				   }).ToList();
				return model;
			});
		...
		[ValidateInput(false)]
		public ActionResult BlitzOrder(int productVariantId, int shoppingCartTypeId, FormCollection form)
		{
			var productVariant = _productService.GetProductVariantById(productVariantId);
			if (productVariant == null)
				return RedirectToRoute("HomePage");
			...
			if (addToCartWarnings.Count > 0)
			{
				...
				//If we got this far, something failed, redisplay form
				var model = PrepareProductDetailsPageModel(productVariant.Product);
				//set already entered values (quantity, customer entered price, gift card attributes, product attributes
				return View(model.ProductTemplateViewPath, model);
			}
		...
		[ChildActionOnly]
		public ActionResult HomepageRecentlyAddedProducts()
		{
			var model = new List<ProductOverviewModel>();
			if (_catalogSettings.RecentlyAddedProductsEnabled)
			{
		...
		public ActionResult HomepageBestSellers(int? productThumbPictureSize)
		{
			...
			{
				Products = PrepareProductOverviewModels(products, prepareSpecificationAttributes: true).ToList()
			};
			return PartialView(model);
		}
